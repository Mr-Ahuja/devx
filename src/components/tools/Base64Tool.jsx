...
export function Base64Tool() {
  ...
  function onModeChange(next) {
    setMode(next)
    updateURL({ tool: 'b64', mode: next })
  }

  function run() {
    setError('')
    try {
      if (mode === 'decode') setOutput(b64d(input))
      else setOutput(b64e(input))
    } catch (e) {
    ...
    <div className="card" style={{ textAlign: 'left' }}>
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 280 }}>
          ...
          <textarea value={input} onChange={e => setInput(e.target.value)} placeholder='Text or base64' style={{ width: '100%', height: 160, padding: 12, borderRadius: 12, background: '#0f151d', color: 'var(--fg)', border: '1px solid #223042' }} />
        </div>
        <div style={{ flex: 1, minWidth: 280 }}>
